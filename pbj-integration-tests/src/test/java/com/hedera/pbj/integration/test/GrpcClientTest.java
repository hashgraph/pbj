...
public class GrpcClientTest {
    ...
    private static class GreeterServiceImpl implements GreeterInterface {
        ...
        public Pipeline<? super HelloRequest> sayHelloStreamRequest(Pipeline<? super HelloReply> replies) {
            // In theory, there must be a single reply for a stream of requests. But we cheat a little
            // because the API doesn't impose this limitation. So we could as well be able to send multiple replies:
            return sayHelloStreamBidi(replies);
        }
    ...
    static void setup() {
        ...
        //        GRPC_CLIENT = new PbjGrpcClient(
        //                webClient,
        //                new PbjGrpcClientConfig(Duration.ofSeconds(10), tls, OPTIONS.authority(),
        // OPTIONS.contentType()));
    }
    ...
    public void testSayHello() {
        //        final GreeterInterface.GreeterClient client = new GreeterInterface.GreeterClient(GRPC_CLIENT,
        // OPTIONS);
        //
    ...
    //                new GreeterInterface.GreeterClient(OPTIONS, GRPC_CLIENT.channel());
    //
    //        final HelloRequest request = HelloRequest.newBuilder().name("test name").build();
    ...
    //    private void testStreamBidi(Function<Pipeline<? super HelloReply>, Pipeline<? super HelloRequest>> method) {
    //        final HelloRequest request = HelloRequest.newBuilder().name("test name").build();
    //        final List<HelloReply> replies = new ArrayList<>();
    //        final List<Throwable> errors = new ArrayList<>();
    //        final AtomicBoolean completed = new AtomicBoolean(false);
    ...
    //        // (in the same JVM, in fact.) So 1 second should be sufficient, unless the computer is really-really
    // slow.
    //        // If we find this not working, then we'll come up with a longer timeout and will be watching the
    // completed
    ...
    //        final GreeterInterface.GreeterClient client =
    //                new GreeterInterface.GreeterClient(OPTIONS, GRPC_CLIENT.channel());
    //        testStreamBidi(client::sayHelloStreamBidi);
    //    }
    //
    //    @Test
    ...
    //        final GreeterInterface.GreeterClient client =
    //                new GreeterInterface.GreeterClient(OPTIONS, GRPC_CLIENT.channel());
    //        testStreamBidi(client::sayHelloStreamRequest);
    //    }
}
