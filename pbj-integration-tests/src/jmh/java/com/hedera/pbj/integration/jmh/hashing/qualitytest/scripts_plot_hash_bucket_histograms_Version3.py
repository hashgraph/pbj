...
Usage:
  python scripts/plot_hash_bucket_histograms.py /path/to/hash_quality_results/run_YYYYMMDD_HHMMSSZ
      [--max-k 400] [--overlay] [--logy]
      [--dpi 300] [--figsize 12x7] [--format svg] [--transparent] [--tight]

Outputs:
  - One image per algorithm: hist_<ALG>.<format>
  - If --overlay: a combined overlay image: hist_overlay.<format>
"""
import argparse
import glob
import json
...
def plot_per_algorithm(
    ...
    out_path = out_dir / f"hist_{sanitize_filename(alg)}.{fmt}"
    fig.savefig(out_path, dpi=dpi, format=fmt, transparent=transparent, bbox_inches="tight" if tight else None)
    plt.close(fig)
...
def plot_overlay(
    ...
    for meta, k, y in alg_results:
        ...
        if normalize:
            y_plot = y / y.sum() if y.sum() > 0 else y
    ...
    if tight:
        fig.tight_layout()
    out_path = out_dir / f"hist_overlay.{fmt}"
    fig.savefig(out_path, dpi=dpi, format=fmt, transparent=transparent, bbox_inches="tight" if tight else None)
    plt.close(fig)
...
def parse_figsize(s: str) -> Tuple[float, float]:
    try:
        w, h = s.lower().replace(" ", "").split("x", 1)
        return float(w), float(h)
    except Exception:
        raise argparse.ArgumentTypeError("figsize must be in the form WIDTHxHEIGHT, e.g., 12x7")
...
def main():
    ...
    parser.add_argument("--dpi", type=int, default=300, help="Output DPI for raster formats (PNG/JPG). Default: 300")
    parser.add_argument("--figsize", type=parse_figsize, default=(10.0, 6.0),
                        help="Figure size in inches as WxH, e.g., 12x7. Default: 10x6")
    parser.add_argument("--overlay-figsize", type=parse_figsize, default=(11.0, 7.0),
                        help="Overlay figure size in inches as WxH. Default: 11x7")
    parser.add_argument("--format", type=str, default="png",
                        choices=["png", "svg", "pdf", "jpg", "jpeg"],
                        help="Output image format. For infinite scalability, use svg or pdf. Default: png")
    parser.add_argument("--transparent", action="store_true", help="Save with transparent background")
    parser.add_argument("--tight", action="store_true", help="Use tight layout and bbox_inches='tight'")
    ...
    for meta_path_str in meta_files:
        ...
        plot_per_algorithm(
            meta, k, y, out_dir,
            show_poisson=True, logy=args.logy,
            figsize=args.figsize, dpi=args.dpi, fmt=args.format,
            transparent=args.transparent, tight=args.tight
        )
    ...
    if args.overlay:
        ...
        plot_overlay(
            aligned, out_dir,
            normalize=True, logy=args.logy,
            figsize=args.overlay_figsize, dpi=args.dpi, fmt=args.format,
            transparent=args.transparent, tight=args.tight
        )
