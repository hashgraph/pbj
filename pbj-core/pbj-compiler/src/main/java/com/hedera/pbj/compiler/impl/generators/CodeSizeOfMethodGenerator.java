 ...
 */
class CodeSizeOfMethodGenerator {

    static String generateSizeOfMethod(final String modelClassName, final List<Field> fields) {
        ...
        return """
                 ...
                 */
                public int sizeOf($modelClass data) {
                    int size = 0;
                    $fieldSizeOfLines
                    return size;
                }
        ...
        if(field.optionalValueType()) {
            ...
            return prefix + switch(field.type()) {
                case ENUM -> "size += sizeOfEnumList(%s, %s);"
                        .formatted(fieldDef, getValueCode);
                case MESSAGE -> "size += sizeOfMessageList($fieldDef, $valueCode, $codec::sizeOf);"
            ...
            return prefix + switch(field.type()) {
                ...
                case STRING -> "size += sizeOfString(%s, %s);"
                        .formatted(fieldDef,getValueCode);
                case MESSAGE -> "size += sizeOfMessage($fieldDef, $valueCode, $codec::sizeOf);"
                        .replace("$fieldDef", fieldDef)
                        .replace("$valueCode", getValueCode)
                        .replace("$codec", ((SingleField)field).messageTypeModelPackage() + "." +
                                Common.capitalizeFirstLetter(field.messageType())+ ".PROTOBUF");
