...
public abstract class PbjCompilerPlugin implements Plugin<Project> {
    ...
    public void apply(Project project) {
        ...
        // Test generation only supported for main source set as base
        final var generatePbjSource = project.getTasks().named("generatePbjSource", PbjCompilerTask.class, t -> {
            t.getGenerateTestClasses().set(pbj.getGenerateTestClasses());
        });
        final var testSrcSet = javaPlugin.getSourceSets().getByName(SourceSet.TEST_SOURCE_SET_NAME);
        testSrcSet.getJava().srcDir(generatePbjSource.flatMap(PbjCompilerTask::getJavaTestOutputDirectory));
    }

    public void pbjSourceSet(SourceSet sourceSet, Project project) {
     ...
     */
    private PbjSourceDirectorySet createPbjSourceDirectorySet(String parentDisplayName, ObjectFactory objects) {
        String name = parentDisplayName + ".pbj";
        String displayName = parentDisplayName + " Pbj source";
        PbjSourceDirectorySet pbjSourceSet =
                objects.newInstance(DefaultPbjSourceDirectorySet.class, objects.sourceDirectorySet(name, displayName));
        pbjSourceSet.getFilter().include("**/*.proto");
        return pbjSourceSet;
    }
}
