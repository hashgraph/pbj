...
An implementation of gRPC for the PBJ runtime, using Helidon, Java 25, and
Virtual Threads.

For the overall gRPC architecture, see [architecture.md](../../../docs/architecture.md#grpc-architecture). For usage examples, see [usage-guide.md](../../../docs/usage-guide.md#grpc-services).
...
request. With Helidon, an entirely new webserver was written with minimal
...
is possible to combine PBJ with the `io.grpc` libraries, however, the `io.grpc`
libraries are large, especially in relation to the size of Helidon itself.
One of the primary objectives of PBJ is to minimize the size and number of
dependencies. In addition, it makes little sense to bolt PBJ onto `io.grpc` and
...
taken in `io.grpc`, but instead of using a library-specific `StreamObserver`
...
Because this gRPC implementation is based on Helidon, each request is
...
Web clients are not able to use HTTP/2 directly. They must use HTTP/1.1.
The `io.grpc` project promotes the use of an Envoy plugin that converts from
HTTP/1.1 to HTTP/2. This is known as gRPC-Web. It may be possible for this
module to support both standard gRPC and gRPC-Web. See
...
This module should provide useful metrics out of the box, such as
the number of times each individual RPC method is called and how long each call
takes. However, we need to be careful not to hurt performance or stability.
