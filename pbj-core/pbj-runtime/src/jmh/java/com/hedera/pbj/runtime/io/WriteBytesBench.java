...
//			fout = new BufferedOutputStream(Files.newOutputStream(tempFileWriting), 1024*1024);
	...
	public void cleanUp() {
		try {
			dataOut.close();
			fout.close();
			Files.delete(tempFileWriting);
			Files.delete(tempFileReading);
		} catch (IOException e){
			e.printStackTrace();
		}
	}
...
//		try (ReadableStreamingData in = new ReadableStreamingData(new BufferedInputStream(Files.newInputStream(tempFileReading), 1024 * 1024)) ) {
