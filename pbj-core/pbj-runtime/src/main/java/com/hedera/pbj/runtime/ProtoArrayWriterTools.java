...
public final class ProtoArrayWriterTools {
    ...
    private static int writeStringNoChecks(
        ...
        bytesWritten += writeTag(output, offset, field, WIRE_TYPE_DELIMITED);
        final int size = sizeOfStringNoTag(value);
        bytesWritten += writeUnsignedVarInt(output, offset + bytesWritten, size);
        System.arraycopy(value, 0, output, offset + bytesWritten, size);
        bytesWritten += size;
        return bytesWritten;
    }
     ...
     * @return the number of bytes written
     */
    public static int writeOptionalBoolean(
            @NonNull final byte[] output, final int offset, @NonNull FieldDefinition field, @Nullable Boolean value) {
        int bytesWritten = 0;
        if (value != null) {
            bytesWritten += writeTag(output, offset, field, WIRE_TYPE_DELIMITED);
            final var newField = field.type().optionalFieldDefinition;
     ...
     * @return the number of bytes written
     */
    public static int writeFixed32(
            @NonNull final byte[] output,
            final int offset,
            @NonNull final FieldDefinition field,
            final int value,
    ...
    public static int writeInt64(
            ...
            final long value,
            boolean skipDefault) {
        assert field.type() == INT64 || field.type() == UINT64 : "Not an Int64 or UInt64 type " + field;
     ...
     * @return the number of bytes written
     */
    public static int writeFixed64(
            @NonNull final byte[] output,
            final int offset,
            @NonNull final FieldDefinition field,
            final long value,
     ...
     * @return the number of bytes written
     */
    public static int writeFloatList(
            @NonNull final byte[] output, final int offset, FieldDefinition field, List<Float> list) {
        assert field.type() == FieldType.FLOAT : "Not a float type " + field;
     ...
     * @return the number of bytes written
     */
    public static int writeBooleanList(
            @NonNull final byte[] output, final int offset, FieldDefinition field, List<Boolean> list) {
        assert field.type() == FieldType.BOOL : "Not a boolean type " + field;
    ...
    public static int writeStringList(
        ...
        for (int i = 0; i < listSize; i++) {
            ...
            curOffset += writeTag(output, curOffset, field, WIRE_TYPE_DELIMITED);
            curOffset += writeUnsignedVarInt(output, curOffset, sizeOfStringNoTag(value));
     ...
     * @return the number of bytes written
     */
    public static int writeByteArrayStringList(
            @NonNull final byte[] output, final int offset, FieldDefinition field, List<byte[]> list) {
        assert field.type() == FieldType.STRING : "Not a string type " + field;
        assert field.repeated() : "Use writeString with non-repeated types";
        // When not a oneOf don't write default value
        if (!field.oneOf() && list.isEmpty()) {
        ...
        for (int i = 0; i < listSize; i++) {
            ...
            curOffset += writeTag(output, curOffset, field, WIRE_TYPE_DELIMITED);
            final int size = sizeOfStringNoTag(value);
            curOffset += writeUnsignedVarInt(output, curOffset, size);
            System.arraycopy(value, 0, output, curOffset, size);
            curOffset += size;
        }
        return curOffset - offset;
    }
