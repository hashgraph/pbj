...
import com.hedera.pbj.compiler.grammar.Protobuf3Lexer;
import com.hedera.pbj.compiler.grammar.Protobuf3Parser;
import com.hedera.pbj.compiler.grammar.Protobuf3Parser.EnumDefContext;
import com.hedera.pbj.compiler.grammar.Protobuf3Parser.MessageDefContext;
import com.hedera.pbj.compiler.grammar.Protobuf3Parser.MessageElementContext;
import com.hedera.pbj.compiler.grammar.Protobuf3Parser.MessageTypeContext;
import com.hedera.pbj.compiler.grammar.Protobuf3Parser.TopLevelDefContext;
...
public final class LookupHelper {
    ...
    public String getUnqualifiedProtoName(final ParserRuleContext context) {
        if (context instanceof final MessageDefContext msgDef) {
            ...
            throw new UnsupportedOperationException(
                    "getUnqualifiedProtoNameForMsgOrEnum only supports MessageDefContext or"
                            + " EnumDefContext");
        }
    }
    ...
    public String getFullyQualifiedProtoName(
        ...
        if (context instanceof final MessageTypeContext msgTypeContext) {
            ...
            for (final var importedProtoFilePath : protoFileImports.get(protoSrcFile.getAbsolutePath())) {
                ...
                if (messageMap == null) {
                    throw new PbjCompilerException(
                            "Failed to find messageMap for proto file ["
                                    + importedProtoFilePath
                                    + "]");
                }
            ...
            // we failed to find
            throw new PbjCompilerException(
                    "Failed to find fully qualified message type for ["
                            + messageType
                            + "] in file ["
                            + protoSrcFile
                            + "] imports = "
            ...
            throw new UnsupportedOperationException(
                    "getPackageForMsgOrEnum only supports MessageDefContext or EnumDefContext");
        }
    }
    ...
    String getPackage(
        ...
        if (context instanceof MessageDefContext
            ...
            throw new UnsupportedOperationException(
                    "getPackageForMsgOrEnum only supports MessageDefContext, "
                            + "EnumDefContext or MessageTypeContext not ["
                        ...
                        if (matchingSrcFiles.size() == 1) {
                            ...
                            throw new PbjCompilerException(
                                    "Import \""
                                            ...
                                            + "\" in proto file \""
                                            + file.getAbsolutePath()
                                            + "\" matched more than 1 file in src files ["
    ...
    private static String getFullyQualifiedProtoNameForMsgOrEnum(
        ...
        if (ruleContext instanceof final Protobuf3Parser.ProtoContext parsedDoc) {
            ...
            thisName =
                    packageStatement.isEmpty() ? "" : packageStatement.get().fullIdent().getText();
            ...
            thisName =
                    getFullyQualifiedProtoNameForMsgOrEnum(enumDef.getParent())
                            + "."
                            + enumDef.enumName().getText();
        } else if (ruleContext instanceof final MessageDefContext msgDef) {
            thisName =
                    getFullyQualifiedProtoNameForMsgOrEnum(msgDef.getParent())
                            + "."
